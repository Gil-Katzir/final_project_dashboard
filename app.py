...
REDIRECT_URL = "https://app.prolific.com/submissions/complete?cc=CVY79URK"
...
# Prolific example:
# ?PROLIFIC_PID=xxx&STUDY_ID=xxx&SESSION_ID=xxx
# -----------------------------
try:
    query_params = st.query_params

    if "PROLIFIC_PID" in query_params and st.session_state.participant_id == "":
        st.session_state.participant_id = str(query_params["PROLIFIC_PID"]).strip()

    if "STUDY_ID" in query_params:
        st.session_state.study_id = str(query_params["STUDY_ID"]).strip()

    if "SESSION_ID" in query_params:
        st.session_state.prolific_session_id = str(query_params["SESSION_ID"]).strip()
...
""", unsafe_allow_html=True)
    ...
    with col_form:
        participant_id_input = str(st.session_state.participant_id).strip()
